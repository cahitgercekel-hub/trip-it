    ...
    <div className="flex flex-col gap-3 p-3">
      <TooltipProvider delayDuration={200}>
        <AnimatePresence mode="wait">
          {!tripGenerated ? (
            /* ─── INPUT VIEW ─── */
            <motion.div key="input-view" {...viewTransition} className="flex flex-col gap-3">
              <HeadingToCard />
              <DatesCard />
              <TripInterestsCard />
            ...
            /* ─── RESULTS VIEW ─── */
            <motion.div key="results-view" {...viewTransition} className="flex flex-col gap-3">
              <button
                onClick={() => setTripGenerated(false)}
                className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors w-fit py-1"
              >
